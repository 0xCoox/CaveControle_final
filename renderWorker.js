...
let alts = { left: false, right: false };
let mode = 0; 
const ray = { origin: new THREE.Vector3(), direction: new THREE.Vector3(), epsilon: 0.005 };
const rayRight = { origin: new THREE.Vector3(), direction: new THREE.Vector3(), epsilon: 0.005 };

let rightHeld = false;
const vrpnController = new VRPNController({
    hands: {
        left: {
            right: ( state ) => { if( state == 0) return; },
            left: ( state ) => { if( state == 0) return; },
            up: ( state ) => { if( state == 0) return; },
            down: ( state ) => { if( state == 0) return; },
            trigger: ( state ) => { console.log(`left hand trigger button ${state}`) },
            mode: ( state ) => { console.log(`left hand mode button ${state}`) },
            stick: ( direction ) => { console.log(`left hand stick ${direction[0]} ${direction[1]}`) },
        ...
        right: {
            right: ( state ) => { if( state == 0) return; console.log(`right hand right button ${state}`) },
            left: ( state ) => { if( state == 0) return; console.log(`right hand left button ${state}`) },
            up: ( state ) => { if( state == 0) return; console.log(`right hand up button ${state}`) },
            down: ( state ) => { if( state == 0) return; console.log(`right hand down button ${state}`) },
            trigger: ( state ) => {
                console.log(`right hand trigger button ${state}`)
                if(state == 1) { rightHeld = true; } else { rightHeld = false; }
            },
            mode: ( state ) => {
                console.log(`right hand mode button ${state}`)
                if(state == 1) flipEyes();
            },
            stick: ( direction ) => { console.log(`right hand stick ${direction[0]} ${direction[1]}`) },
